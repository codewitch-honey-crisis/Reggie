...
namespace Reggie {
    internal partial class CSharpCompiledTokenizerGenerator {
...
if(blockEnds[sacc]==null) {        
    if(0==(symbolFlags[sacc]&1)) {
        if(lineCounted) {
            ...
            Response.Write("\r\n            yield return new ");
            Response.Write(codetoken);
            Response.Write("(absoluteIndex, sb.Length, position, (int)(cursorPos-position), ");
            Response.Write(symbolTable[sacc]);
            Response.Write(", sb.ToString(), line, column);");

            } else {
            Response.Write("\r\n            yield return (AbsolutePosition: absoluteIndex, AbsoluteLength: sb.Length, Position: position, Length: (int)(cursorPos-position), SymbolId: ");
            Response.Write(symbolTable[sacc]);
            Response.Write(", Value: sb.ToString(), Line: line, Column: column);");
...
} else // if(blockEnds[sacc]...
        {
            ...
            Response.Write(curname);
            Response.Write(",sb, ref cursorPos, ref absi, ref lc, ref cc, tabWidth, ref ch)) {");
 ...
 // if(_Tokenize...
            ...
            Response.Write("\r\n            yield return new ");
            Response.Write(codetoken);
            Response.Write("(absoluteIndex, sb.Length, position, (int)(cursorPos-position), ERROR, sb.ToString(), line, column);");
...
if(string.IsNullOrEmpty(codetoken)) {
            ...
            Response.Write("\r\n        yield return new ");
            Response.Write(codetoken);
            Response.Write("(errorIndex, sb.Length, errorPos, (int)(popsition- errorPos+1), -1, sb.ToString()");
            Response.Write(lineCounted?", errorLine, errorColumn":"");
            Response.Write(");");
...
foreach(var rule in rules) {
    ...
    if(be!=null) {
        if(!lineCounted) {
            ...
            Response.Write(" ");
            Response.Write(curname);
            Response.Write(", System.Text.StringBuilder sb, ref long cursorPos, ref long absi, ref int lc, ref int cc, int tabWidth, ref int ch) {");
