...
-- Test.rgg specification file on 11/11/2021 7:34:21 AM UTC
use [Test]
GO
...
DROP PROCEDURE [dbo].[SqlCompiledMatcher_MatchCommentBlockBlockEnd]
GO
CREATE PROCEDURE [dbo].[SqlCompiledMatcher_MatchCommentBlockBlockEnd] @value NVARCHAR(MAX), @ch BIGINT, @tch BIGINT, @ch1 NCHAR, @ch2 NCHAR, @cursorPos BIGINT, @index INT, @absi BIGINT, @valueEnd INT, @capture NVARCHAR(MAX), @newIndex INT OUTPUT, @newCursorPos BIGINT OUTPUT, @newAbsi BIGINT OUTPUT, @newCapture NVARCHAR(MAX) OUTPUT, @newCh BIGINT OUTPUT, @newTch BIGINT OUTPUT, @newCh1 NCHAR OUTPUT, @newCh2 NCHAR OUTPUT
AS
BEGIN
    DECLARE @adv INT
    WHILE @ch <> -1
...
BEGIN
    ...
    q1:
        ...
        GOTO next
    q2:
        EXEC @len = [dbo].[SqlCompiledMatcher_MatchCommentBlockBlockEnd] @value = @value, @ch = @ch,  @tch = @tch, @ch1 = @ch1, @ch2 = @ch2, @cursorPos = @cursorPos, @index = @index, @absi = @absi, @valueEnd = @valueEnd, @capture = @capture, @newIndex = @newIndex OUTPUT, @newAbsi = @newAbsi OUTPUT, @newCursorPos = @newCursorPos OUTPUT, @newCapture = @newCapture OUTPUT, @newCh = @newCh OUTPUT, @newTch = @newTch OUTPUT, @newCh1 = @newCh1 OUTPUT, @newCh2 = @newCh2 OUTPUT
        SET @index = @newIndex
        SET @absi = @newAbsi
