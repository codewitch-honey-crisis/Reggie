...
-- ExampleLexer.rgg specification file on 11/8/2021 4:42:48 AM UTC
use [Test]
GO
-- <summary>Represents a lexer/tokenizer for the regular expressions in ExampleLexer.rgg</summary>
DROP TABLE [dbo].[ExampleTokenizeState]
GO
CREATE TABLE [dbo].[ExampleTokenizeState] (
    [StateId]  INT NOT NULL,
    [AcceptId] INT NOT NULL DEFAULT -1,
    [BlockEndId] INT NOT NULL DEFAULT -1
    CONSTRAINT [PK_ExampleTokenizeState] PRIMARY KEY ([StateId], [BlockEndId])
)
GO

DROP TABLE [dbo].[ExampleTokenizeStateTransition]
GO

CREATE TABLE [dbo].[ExampleTokenizeStateTransition]
(
    [StateId] INT NOT NULL , 
    ...
    [Min] BIGINT NOT NULL, 
    [Max] BIGINT NOT NULL, 
    CONSTRAINT [PK_ExampleTokenizeStateTransition] PRIMARY KEY ([StateId], [BlockEndId], [Min], [Max]) 
)
GO
DROP TABLE [dbo].[ExampleTokenizeSymbol]
GO

CREATE TABLE [dbo].[ExampleTokenizeSymbol] (
    [Id] INT NOT NULL,
...
DROP PROCEDURE [dbo].[Example_TokenizeCommentBlockBlockEnd]
GO
CREATE PROCEDURE [dbo].[Example_TokenizeCommentBlockBlockEnd] @value NVARCHAR(MAX), @ch BIGINT, @tch BIGINT, @ch1 NCHAR, @ch2 NCHAR, @cursorPos BIGINT, @absi BIGINT, @index INT, @valueEnd INT, @capture NVARCHAR(MAX), @newIndex INT OUTPUT, @newCursorPos BIGINT OUTPUT, @newAbsi INT OUTPUT, @newCapture NVARCHAR(MAX) OUTPUT, @newCh BIGINT OUTPUT, @newTch BIGINT OUTPUT, @newCh1 NCHAR OUTPUT, @newCh2 NCHAR OUTPUT
AS
BEGIN
    DECLARE @adv INT = 0
    DECLARE @matched INT
...
GO
-- <summary>Lexes tokens off of <paramref name="value"/></summary>
...
CREATE PROCEDURE [dbo].[Example_Tokenize] @value NVARCHAR(MAX), @position BIGINT = 0
AS
BEGIN
    DECLARE @adv INT
    ...
    q3:
        ...
        BEGIN
            INSERT INTO #Results SELECT @absoluteIndex AS [AbsolutePosition], CAST((@absi - @absoluteIndex) AS INT) AS [AbsoluteLength], @position AS [Position], CAST((@cursorPos - @position) AS INT) AS [Length], 1 AS [SymbolId], @capture AS [Value]
        END
        SET @capture = N''
        CONTINUE
    q4:
    ...
    q8:
        ...
        IF DATALENGTH(@capture) > 0
        BEGIN
            INSERT INTO #Results SELECT @absoluteIndex AS [AbsolutePosition], CAST((@absi - @absoluteIndex) AS INT) AS [AbsoluteLength], @position AS [Position], CAST((@cursorPos - @position) AS INT) AS [Length], 2 AS [SymbolId], @capture AS [Value]
        END
        SET @capture = N''
        CONTINUE
    q9:
    ...
    q12:
        ...
        IF DATALENGTH(@capture) > 0
        BEGIN
            INSERT INTO #Results SELECT @absoluteIndex AS [AbsolutePosition], CAST((@absi - @absoluteIndex) AS INT) AS [AbsoluteLength], @position AS [Position], CAST((@cursorPos - @position) AS INT) AS [Length], 4 AS [SymbolId], @capture AS [Value]
        END
        SET @capture = N''
        CONTINUE
    q13:
    ...
    q18:
        ...
        END
        EXEC @len = [dbo].[Example_TokenizeCommentBlockBlockEnd] @value = @value, @ch = @ch,  @tch = @tch, @ch1 = @ch1, @ch2 = @ch2, @cursorPos = @cursorPos, @absi = @absi, @index = @index, @valueEnd = @valueEnd, @capture = @capture, @newIndex = @newIndex OUTPUT, @newCursorPos = @newCursorPos OUTPUT, @newAbsi = @newAbsi OUTPUT, @newCapture = @newCapture OUTPUT, @newCh = @newCh OUTPUT, @newTch = @newTch OUTPUT, @newCh1 = @newCh1 OUTPUT, @newCh2 = @newCh2 OUTPUT
        SET @index = @newIndex
        SET @absi = @newAbsi
        ...
        IF @len <> 0
        BEGIN
            INSERT INTO #Results SELECT @absoluteIndex AS [AbsolutePosition], CAST((@absi - @absoluteIndex) AS INT) AS [AbsoluteLength], @position AS [Position], CAST((@cursorPos - @position) AS INT) AS [Length], 40 AS [SymbolId], @capture AS [Value]
            SET @capture = N''
            CONTINUE
        END -- IF not matched
        INSERT INTO #Results SELECT @absoluteIndex AS [AbsolutePosition], DATALENGTH(@capture) / 2 AS [AbsoluteLength], @position AS [Position], CAST((@cursorPos - @position) AS INT) AS [Length], -1 AS [SymbolId], @capture AS [Value]
    ...
    q19:
        ...
        IF DATALENGTH(@capture) > 0
        BEGIN
            INSERT INTO #Results SELECT @absoluteIndex AS [AbsolutePosition], CAST((@absi - @absoluteIndex) AS INT) AS [AbsoluteLength], @position AS [Position], CAST((@cursorPos - @position) AS INT) AS [Length], 3 AS [SymbolId], @capture AS [Value]
        END
        SET @capture = N''
        CONTINUE
    q20:
    ...
    q43:
        ...
        IF DATALENGTH(@capture) > 0
        BEGIN
            INSERT INTO #Results SELECT @absoluteIndex AS [AbsolutePosition], CAST((@absi - @absoluteIndex) AS INT) AS [AbsoluteLength], @position AS [Position], CAST((@cursorPos - @position) AS INT) AS [Length], 0 AS [SymbolId], @capture AS [Value]
        END
        SET @capture = N''
        CONTINUE
    q44:
        ...
        IF DATALENGTH(@capture) > 0
        BEGIN
            INSERT INTO #Results SELECT @absoluteIndex AS [AbsolutePosition], CAST((@absi - @absoluteIndex) AS INT) AS [AbsoluteLength], @position AS [Position], CAST((@cursorPos - @position) AS INT) AS [Length], 6 AS [SymbolId], @capture AS [Value]
        END
        SET @capture = N''
        CONTINUE
    error:
        IF @matched = 0 AND @hasError = 1
        BEGIN
        ...
        SET @hasError = 1
    END -- WHILE input loop
    IF @hasError = 0 AND DATALENGTH(@capture) > 0
    BEGIN
        INSERT INTO #Results SELECT @errorIndex AS [AbsolutePosition], DATALENGTH(@capture) / 2 AS [AbsoluteLength], @errorPos AS [Position], CAST((@cursorPos - @errorPos) AS INT) AS [Length], -1 AS [SymbolId], @capture AS [Value]
